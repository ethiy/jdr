    ...
    \section{Experiments}
        ...
        \begin{frame}{Transferability}
            ...
            \only<2|handout:0>{
                ...
                \end{table}
                \begin{center}
                    \footnotesize \underline{Reminder:}\\
                    \scriptsize (F || B)XY $\rightarrow$ (Facet || Building) <X> <Y>.\\
                    X(O || U)S $\rightarrow$ <X> (Over || Under) Segmentation.\\
                    XIT $\rightarrow$ <X> Inaccurate Topology.\\
                    XI(B || G) $\rightarrow$ <X> Imprecise (Borders || Geometry).
                \end{center}
            }
            \only<3-|handout:2>{
                \begin{table}
