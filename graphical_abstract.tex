...
\begin{document}
    \begin{tikzpicture}
    	\node (residual) at (0,0) {\includegraphics[height=1cm, angle=270]{images/graph_abstract/residual}};
        ...
        \path (residual.east) + (.8,0) node (ortho_projection) {\includegraphics[height=1cm, angle=270]{images/graph_abstract/orthoprojection}};
        ...
        \path (errors |- residual_legend) node (errors_legend) {\tiny (e) Error detection:};
        \path (errors_legend.south) + (0,-.1) node[align=left] (errors_list) {\tiny {\color{red}$\blacksquare$ \texttt{FIB}}.};
         \path (errors_list.south) node[align=left] (errors_list) {\tiny {\color{blue}$\blacksquare$ \texttt{FOS}}.};
